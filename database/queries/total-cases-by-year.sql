...
-- total jumlah kasus baru (terinfeksi) akibat COVID-19 per tahun
...
SELECT
    ...
    SUM(new_cases) AS total_infeksi_tahunan -- Menghitung jumlah total nilai dalam kolom 'new_cases'
                                        -- untuk setiap tahun yang dikelompokkan,
                                        -- dan memberikan alias 'total_infeksi_tahunan' untuk kolom hasil.
FROM
...
ORDER BY
    tahun                               -- Mengurutkan hasil query berdasarkan kolom 'tahun'
                                        -- secara menaik (ascending), sehingga total infeksi
